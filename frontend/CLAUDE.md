...
**Типы недели:** `odd` (числитель), `even` (знаменатель), `both` (каждая)

---

## Управление состоянием

- **Zustand** — только для UI-состояния: модалки, флаги, локальные настройки интерфейса
- **TanStack Query** — серверные данные: загрузка, кэш, инвалидация, мутации

Не хранить серверные данные в Zustand и не дублировать их туда вручную.
